...
with tab_text:
    ...
    st.write('3.    Subjects start behind one of the lines. Start the audio player. The audio gives a 5 second warning then a 3 beep ascending tone starts the test. The subject runs to the other line 20m away. They must reach the other line by the time the audio next beeps (9 seconds after the start). The subject continues running 20m shuttles from one line to the other in time with the beeps. If the subject reaches the other line before the next beep they must wait for the beep before setting off back. Subjects only have to \'toe the line\' i.e. touching the line with a foot is good enough, there is no requirement for them to completely cross it. At each beep, the audio will read out a level and shuttle number e.g. *"Level 1, 1"* at the beginning of the test, *"Level 1, 2"* at the next beep and so on.')
    st.write('4.    Every minute the audio will play a 3 beep ascending tone and announce the start of the next level. The time between the beeps reduces at every change in level so the subject has to run progressively faster at each change of level. Eventually a point will be reached where the subject has difficulty reaching the opposite line before the next beep. When the subject fails to make the line before the next beep twice in a row the test ends. Note the level and shuttle number *for the last shuttle that was successfully completed* and [enter them below](#result) to see your results.')
...
if shuttle:
    ...
    if check_level:
        ...
        mas_string = '<strong><em>Your maximal aerobic speed (MAS), the speed you were running at when the test ended, is <span style="color:#F63366;"> ' + "%.1f" % mas + '&nbsp;km&nbsp;h\u207B\u00B9</span><br> (which is <span style="color:#F63366;">' + "%.1f" % (mas/3.6) + '&nbsp;m&nbsp;s\u207B\u00B9)</span></em></strong>'
